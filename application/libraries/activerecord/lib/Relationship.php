    ...
    public function load(Model $model) {
        ...
        if (!isset($this->initialized))
        {
            if ($this->through)
            {
                ...
                {
//                    $this->get_table()->clear_cache(NULL);
//                    echo "<pre>";
//                    print_r($this->get_table()->get_relationships());
//                    echo "</pre>";
                    if (!($through_relationship = $this->get_table()->get_relationship(lcfirst(str_ireplace("AR\\", "", $this->through)))))
                    {
                ...
                if (!isset($through_table))
                {
                    $through_table = Table::load(classify($this->through, false));
                }
    ...
    public function load_eagerly($models = array(), $attributes = array(), $includes, Table $table) {
        ...
        {
            $rel_pk = array($this->primary_key[0]);
        }
        else
        {
            $rel_pk = $table->pk;
        }
        $this->query_and_attach_related_models_eagerly($table, $models, $attributes, $includes, $this->foreign_key, $rel_pk);
    }
