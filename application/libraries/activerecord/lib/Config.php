...
class Config extends Singleton {
    ...
    public static function initialize(Closure $initializer) {
        return $initializer(parent::instance());
    }
    ...
    public function set_connections_from_system($dbnames, $default_connection = null) {
        if (!is_array($dbnames))
            throw new ConfigException("DbNames must be an array");
        $connections = array();

        foreach ($dbnames as $k => $v) {
            $db = \WolfMVC\Registry::get("database_" . $v);
            if (!($db instanceof \WolfMVC\Database\Connector\Mysql))
            {
                throw new \Exception("Impossibile trovare database " . $v . " tra quelli registrati nel sistema", 0, NULL);
                return null;
            }
            $connections[$k] = "mysql://{$db->getUsername()}:{$db->getPassword()}@{$db->getHost()}/{$db->getSchema()}";
        }
//        if ($default_connection)
//            $this->set_default_connection($default_connection);
        $this->set_connections($connections, $default_connection);
    }
