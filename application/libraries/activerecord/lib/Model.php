...
require_once APP_PATH . '/wolfmvc/model.wolfMVC.php'; //includo manualmente perché l'autoload non parte su questa interfaccia - non so perché
...
class Model implements ModelInt {
    ...
    /**
     * Used to set standard configurations for the model.
     * Example: in the model definition
     * <code>
     * static $confs = array(
     *      "start_conf" => array("field1" => true, "field2" => false)
     * );
     *
     * </code>
     * usage:
     * <code>
     * $mod = \ModelClass::find(1234);
     * $mod->setPreConf("start_conf");
     * // now $mod has the pre-set values;
     * $mod->save(); //because setPreConf doesn't save to db!!
     * </code>
     * @var array
     */
    static $confs = array();
...
//do not remove - have to return null by reference in strict mode
     ...
     */
    public function attributes($aliased = false) {
        if ($aliased)
        {
            $aliases = static::$alias_attribute;
            $attr = array();
            foreach ($this->attributes as $k => $v) {
                $nk = array_search($k, $aliases);
                if ($nk !== FALSE)
                {
                    $attr[$nk] = $v;
                }
     ...
     *
     * @param array $attributes Array containing attribute names or aliases
    ...
    public function get_values_for($attributes) {
        ...
        foreach ($attributes as $name) {
            ...
            {
                $ret[$name] = $this->attributes[$aliases[$name]];
            }
        }
        return $ret;
    }
...
// anything left in $args is a find by pk
    ...
    public function setPreConf($confName){
        if (!isset(static::$confs) || !is_array(static::$confs)||!isset(static::$confs[$confName])){
            return false;
        }
        return $this->set_attributes(static::$confs[$confName]);
    }
