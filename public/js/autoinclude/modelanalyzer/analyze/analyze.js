(function () {
    "use strict";
    var app = angular.module("Analyze", ['DATAMODULE', 'THEMASK', 'SMARTBUTTON', 'COMMON', 'TREEMENULIB'], function ($interpolateProvider) {
        $interpolateProvider.startSymbol("--__");
        $interpolateProvider.endSymbol("__--");
    });
    app.controller("AnalyzeController", function (DataService, $scope, $rootScope, $timeout, TheMask, $interval, SmartButton, Common) {
        var that = this; 
        Common.pageOfficer.linkCtrl(this);
        Common.pageOfficer.attachToScope($scope);
        $scope.sNodeConnector = {label: null};
 
        $scope.treeNav = {}; 
        $scope.$watch("sNodeConnector", function (newValue, oldValue) {
            if (oldValue.label === null || newValue.label === null) {
                return;
            }
            ...
            if (newValue.label !== oldValue.label) {
                $scope.sNodeConnector.unconfirmed = true;
                return $scope.treeNav.updateNodeView($scope.sNodeConnector);
            }
...
//            that.mascheraCercaLead[1].open();
//        }
//        this.ricercaLead = function () {
//            var search = $scope.opPar.search;
//            $scope.opPar.timerRicerca = $timeout(function () {
