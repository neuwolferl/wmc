...
//                        console.log(that.hash[ticketHash].response.data);
            ...
            return {
                    ...
                    if (typeof (url) === "undefined" && that.conf.loaded) {
                        ...
                        var responsePromise = $http.get(url)
                                ...
                                .error(function (data, status, headers, config) {
                                    ...
                                    throw Error("DataModule configuration failed");
                                    if (typeof (errorCallback) === "function")
                                        return errorCallback(data, status, headers, config);
                                });
                        return true;
                    }
                },
                ...
                getData: function (op, queryPars, message, messagePars, successCallback, errorCallback, timedCallback) {
//                result.result = "pending";
                    if (that.conf.loaded) {
                        if (!that.conf || !that.conf.config || !that.conf.config || !that.conf.config.services) {
                            throw Error("No valid configuration!");
                            return;
                        }
                        if (!that.conf.config.services[op]) {
                            throw Error("No valid configuration! Missing ", op, that.conf.config.services);
                            return;
                        }
                        var url = that.conf.config.services[op];
                        if (!url || typeof (url) !== "string") {
                            console.error("Errore - servizio " + op + " non disponibile");
                            throw Error("Errore - servizio " + op + " non disponibile");
                            return;
                        }
                        else {
                            var ticket = that.setTicket();
                            ...
                            ticket[1].status = "pending";
                            ticket[1].reqTimestamp = new Date().getTime();
                            if (timedCallback && angular.isFunction(timedCallback)) {
                                var timed = {index: 0, intervalRef: null, stopRequestEffectsCbk: null};
                                timed.stopRequestEffectsCbk = function () {
                                        ticket[1].status = "aborted";
                                        $interval.cancel(timed.intervalRef);
                                    };
                                timed.intervalRef = $interval(function () {
                                    timedCallback(timed, function () {
                                        $interval.cancel(timed.intervalRef);
                                    });
                                    timed.index++;
                                    if (ticket[1].status !== "pending") {
                                        $interval.cancel(timed.intervalRef);
                                    }
                                }, 1000);
                            }
...
//                            data: 'data=' + JSON.stringify(dataObject),
                ...
                postData: function (op, dataObj, message, messagePars, successCallback, errorCallback,timedCallback) {
